...
MEAN_INDIV_FEMALE_RATIO = True
MEAN_INDIV_BMI_RATIO = 25
MEAN_INDIV_SMOKER_RATIO = 0
...
def test():
    path_pairs = Path(getenv("INPUT_PAIRS"))
    path_definitions = Path(getenv("INPUT_DEFINITIONS"))
    path_dense_fevents = Path(getenv("INPUT_DENSE_FEVENTS"))
    path_info = Path(getenv("INPUT_INFO"))
    output_path = Path(getenv("OUTPUT"))
    timings_path = Path(getenv("TIMINGS"))
    # Initialize the CSV output
    line_buffering = 1

    os.chdir("/finngen/red/cabg_sex")
    ...
    # Exclusions
    logger.info("Exlude females: {EXCLUDE_FEMALES}, exclude males: {EXCLUDE_MALES}")
    ...
    while not jobs.empty():
        ...
        if EXCLUDE_FEMALES or EXCLUDE_MALES:
            if is_sex_specific:
                if sex_specific_value == 1 and EXCLUDE_MALES:
                    logger.info("Skipping sex spesific model")
                    continue
                if sex_specific_value == 2 and EXCLUDE_FEMALES:
                    logger.info("Skipping sex spesific model")
                    continue
        ...
        try:
            ...
            coxformula = "BIRTH_TYEAR + SMOKER + BMI + prior*female"
            if (is_sex_specific):
                coxformula = "BIRTH_TYEAR + SMOKER + BMI + prior"
...
def main(path_pairs, path_definitions, path_dense_fevents, path_info, output_path, timings_path):
    ...
    # Exclusions
    logger.info(f"Exlude females: {EXCLUDE_FEMALES}, exclude males: {EXCLUDE_MALES}")
    ...
    while not jobs.empty():
        ...
        if EXCLUDE_FEMALES or EXCLUDE_MALES:
            if is_sex_specific:
                logger.info(f"Skipping sex specific model {pair}")
            is_sex_specific = True
...
def load_data(path_pairs, path_definitions, path_dense_fevents, path_info):
    ...
    # Get endpoint list
    endpoints = pd.read_csv(path_definitions, delimiter = "\t", usecols=["NAME", "SEX"])

    # Get first events
    df_events = pd.read_csv(path_dense_fevents)

    # Get sex and approximate birth date of each indiv
    df_info = pd.read_csv(path_info, delimiter = "\t", usecols=["FINNGENID", "BL_YEAR", "BL_AGE", "SEX", "HEIGHT", "WEIGHT", "SMOKE3"])
    df_info = df_info.dropna(subset = ['SMOKE3', 'HEIGHT', 'WEIGHT'])
    df_info["female"] = df_info.SEX == "female"
    df_info["BIRTH_TYEAR"] = df_info.BL_YEAR - df_info.BL_AGE
    df_info.loc[:, "SMOKER"] = df_info.loc[:, "SMOKE3"].replace({"current": 1, "former": 0, "never": 0})
    df_info["BMI"] = df_info.WEIGHT*(df_info.HEIGHT/100)**-2
    df_info = df_info.drop(columns=["SEX", "BL_YEAR", "BL_AGE", "SMOKE3", "HEIGHT", "WEIGHT"])

    if EXCLUDE_FEMALES and EXCLUDE_MALES:
        logger.error("All participants excluded: nothing to analyze")
    
    if EXCLUDE_FEMALES:
        df_info = df_info[df_info.female == False]
    if EXCLUDE_MALES:
        df_info = df_info[df_info.female == True]
...
def prep_lifelines(df_unexp, df_unexp_death, df_unexp_exp_p1, df_unexp_exp_p2, df_tri_p1, df_tri_p2):
    ...
    keep_cols = ["duration", "prior", "BIRTH_TYEAR", "female", "outcome", "weight", "BMI", "SMOKER"]
...
def compute_coxhr(pair, df, lag, step_size, is_sex_specific, nindivs, res_writer, coxformula):
    ...
    mean_indiv = pd.DataFrame({
        ...
        "BMI": [MEAN_INDIV_BMI_RATIO],
        "SMOKER": [MEAN_INDIV_SMOKER_RATIO]
    })
    if is_sex_specific:
    ...
    cph.params_
    prior_coef = cph.params_["prior[T.True]"]
    prior_se = cph.standard_errors_["prior[T.True]"]
    prior_hr = np.exp(prior_coef)
    prior_ci_lower = np.exp(prior_coef - 1.96 * prior_se)
    prior_ci_upper = np.exp(prior_coef + 1.96 * prior_se)
    prior_pval = cph.summary.p["prior[T.True]"]
    prior_zval = cph.summary.z["prior[T.True]"]
    prior_norm_mean = norm_mean["prior[T.True]"]
    ...
    if not is_sex_specific:
        sex_coef = cph.params_["female[T.True]"]
        sex_se = cph.standard_errors_["female[T.True]"]
        sex_hr = np.exp(sex_coef)
        sex_ci_lower = np.exp(sex_coef - 1.96 * sex_se)
        sex_ci_upper = np.exp(sex_coef + 1.96 * sex_se)
        sex_pval = cph.summary.p["female[T.True]"]
        sex_zval = cph.summary.z["female[T.True]"]
        sex_norm_mean = norm_mean["female[T.True]"]
    else:
        sex_coef = np.nan
        sex_se = np.nan
        sex_hr = np.nan
    ...
    if 'prior[T.True]:female[T.True]' in cph.params_:
        sexprior_coef = cph.params_["prior[T.True]:female[T.True]"]
        sexprior_se = cph.standard_errors_["prior[T.True]:female[T.True]"]
        sexprior_hr = np.exp(sexprior_coef)
        sexprior_ci_lower = np.exp(sexprior_coef - 1.96 * sexprior_se)
        sexprior_ci_upper = np.exp(sexprior_coef + 1.96 * sexprior_se)
        sexprior_pval = cph.summary.p["prior[T.True]:female[T.True]"]
        sexprior_zval = cph.summary.z["prior[T.True]:female[T.True]"]
        sexprior_norm_mean = norm_mean["prior[T.True]:female[T.True]"]
    else:
        sexprior_coef = np.nan
        sexprior_se = np.nan
        sexprior_hr = np.nan
        sexprior_ci_lower = np.nan
        sexprior_ci_upper = np.nan
        sexprior_pval = np.nan
        sexprior_zval = np.nan
        sexprior_norm_mean = np.nan
    ...
    smoker_norm_mean = norm_mean["SMOKER"]
    ...
    nobservations = "{:g}".format(cph.weights.sum())
    nevents="{:g}".format(cph.weights[cph.event_observed > 0].sum())
